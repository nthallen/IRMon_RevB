classdef subbusd_slcan < handle
  ...
  properties
    % The suggested port name
    port
    % The serial port object
    s
    % The current request ID number
    cur_req_ID
    % If true, provides useful output during open and close.(true)
    verbose_fileio
    % If true, prints slcan packets that are sent or received. (false)
    verbose_slcan
    % If true, prints subbus CAN messages that are sent or received. (false)
    verbose_sbslcan
  end
